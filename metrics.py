...
def accuracy(logit, label):
    corr = tf.equal(tf.argmax(logit, -1), tf.argmax(label, -1))
    accr = tf.reduce_mean(tf.cast(corr, tf.float32), name='accuracy')
    return accr
...
def confusion_matrix(logit, y):
    logit = tf.one_hot(tf.argmax(logit, axis=-1), depth=y.get_shape().as_list()[3])
